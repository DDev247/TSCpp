...
struct write_location {
  ...
  */
  ...
  {
    if (EEPROMReadRaw(address)!=value)
    {
      EEPROMWriteRaw(address, value);
      ++counter;
    }
  }
...
void EEPROMWriteRaw(uint16_t address, uint8_t data) { TSCpp::instance->EEPROM.write(address, data); }
uint8_t EEPROMReadRaw(uint16_t address) { return TSCpp::instance->EEPROM.read(address); }
...
static inline eeprom_address_t load(table_axis_iterator it, eeprom_address_t address)
{
  ...
  while (!it.at_end())
  {
    *it = pConverter->from_byte(EEPROMReadRaw(address));
    ++address;
    ++it;
  }
  return address;    
...
{
  EEPROMWriteRaw(compute_crc_address(pageNum), crcValue);
}
...
{
  eeprom_address_t targetAddress = compute_crc_address(pageNum);
  ...
  buffer.raw[0] = EEPROMReadRaw(targetAddress + 0);
  buffer.raw[1] = EEPROMReadRaw(targetAddress + 1);
  buffer.raw[2] = EEPROMReadRaw(targetAddress + 2);
  buffer.raw[3] = EEPROMReadRaw(targetAddress + 3);

  return  ((buffer.value >> 24) & 0xffU) | // move byte 3 to byte 0
      ((buffer.value << 8) & 0xff0000U) | // move byte 1 to byte 2
      ((buffer.value >> 8) & 0xff00U) | // move byte 2 to byte 1
      ((buffer.value << 24) & 0xff000000U);
}
...
void storeCalibrationCRC32(uint8_t calibrationPageNum, uint32_t calibrationCRC)
{
  ...
  EEPROMWriteRaw(targetAddress, calibrationCRC);
}
...
byte readLastBaro(void) { return EEPROMReadRaw(EEPROM_LAST_BARO); }
/// Write last acquired arometer reading to EEPROM.
void storeLastBaro(byte newValue) { EEPROMWriteRaw(EEPROM_LAST_BARO, newValue); }
/// Read EEPROM current data format version (from offset EEPROM_DATA_VERSION).
byte readEEPROMVersion(void) { return EEPROMReadRaw(EEPROM_DATA_VERSION); }
/// Store EEPROM current data format version (to offset EEPROM_DATA_VERSION).
void storeEEPROMVersion(byte newVersion) { EEPROMWriteRaw(EEPROM_DATA_VERSION, newVersion); }
