...
static inline page_iterator_t map_page_offset_to_entity(uint8_t pageNumber, uint16_t offset)
{
  ...
  switch (pageNumber)
  {
    ...
    case veSetPage: 
    {
      CHECK_RAW(veSetPage, offset, &configPage2, sizeof(config2), 0)
      END_OF_PAGE(veSetPage, 1)
    }

    case ignSetPage: 
    {
      CHECK_RAW(ignSetPage, offset, &configPage4, sizeof(config4), 0)
      END_OF_PAGE(ignSetPage, 1)
    }
    
    case afrSetPage: 
    {
      CHECK_RAW(afrSetPage, offset, &configPage6, sizeof(config6), 0)
      END_OF_PAGE(afrSetPage, 1)
    }

    case canbusPage:  
    {
      CHECK_RAW(canbusPage, offset, &configPage9, sizeof(config9), 0)
      END_OF_PAGE(canbusPage, 1)
    }

    case warmupPage: 
    {
      CHECK_RAW(warmupPage, offset, &configPage10, sizeof(config10), 0)
      END_OF_PAGE(warmupPage, 1)
    }

    case progOutsPage: 
    {
      CHECK_RAW(progOutsPage, offset, &configPage13, sizeof(config13), 0)
    ...
    case boostvvtPage2: //Boost, VVT and staging maps (all 8x8)
    {
      ...
      CHECK_RAW(boostvvtPage2, offset, &configPage15, sizeof(config15), 1)
